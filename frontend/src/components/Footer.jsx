...
export default function Footer() {
  ...
  return (
    <footer className="border-t border-gray-200 dark:border-gray-800 bg-lightCard dark:bg-darkCard">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          {footer.footerSkillsList?.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-4">
              {footer.footerSkillsList.map(s => (
                <span key={s} className="text-[10px] px-2 py-1 rounded-full bg-accent/10 text-accent">{s}</span>
              ))}
            </div>
          )}
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm">
            <li className="flex items-center gap-2"><Phone size={14} className="text-accent" /> {contact.adminPhone}</li>
            <li className="flex items-center gap-2"><Mail size={14} className="text-accent" /> {contact.adminEmail}</li>
            <li className="flex items-center gap-2"><MapPin size={14} className="text-accent" /> {contact.adminLocation}</li>
          </ul>
        </div>
      </div>
